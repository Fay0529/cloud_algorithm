...
def loadData(path):
    ...
    return df
df=loadData('/home/fay/workspace/python/select_company_name_company_description_.tsv')
...
dfTest=df1
#载入自定义词典
jieba.load_userdict('/home/fay/workspace/python/dict.txt')
#载入停词表
jieba.analyse.set_stop_words('/home/fay/workspace/python/stopwords.txt')
#print(type(dfTest))
try:
    count=1
    f = open("/home/fay/workspace/python/out.txt", "w")
    for i in dfTest.index:
        ...
        for word in chinese_keywords:
            ...
            cursor.execute(sql)
            db.commit()
            print('%s,%s,%s'%(company_name,word[0],word[1]*5),file=f)
