...
def main(sc):
    for i in range(1,23):
        path="file:///home/faychen/bigdata/input/class%s" % i
        print('**********************for class %s***************************'%i,file=f)
        ...
        #下面对数据进行特征提取, "You have an error in your SQL syntax; check the manual that corresponds to your MySQL server version for the right syntax to use near ') VALUES (1,1.4644998365793143e-29,3.661249591448371e-29,8.700252159357907e-28,1' at line 1")
        ...
        print('四元素权重:[%s\n,%s\n,%s\n,%s\n]'%(w1,w2,w3,w4),file=f)
        sql="INSERT INTO salary_influence_factors(class,company_nature,education_degree,job_exp,job_city) VALUES (%s,%s,%s,%s,%s)" % (i,abs(w1*1e3),abs(w2*1e3),abs(w3*1e3),abs(w4*1e3))
        cursor.execute(sql)
        db.commit()
